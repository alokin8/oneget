...
namespace Microsoft.OneGet.Core.AppDomains {
    ...
    internal class PluginAssemblyResolver : MarshalByRefObject {
        ...
        public Assembly Resolve(object sender, ResolveEventArgs args) {
            ...
            return (from folder in _searchPath
                from assemblyPath in _assemblyFileExtensions
                    .Select(extension => Path.Combine(folder, ("{0}.{1}".format(name, extension))))
                    .Where(File.Exists)
                select _loadWhenResolving(assemblyPath)).FirstOrDefault();
        }
        ...
        internal Assembly LoadBinary(string assemblyPath) {
            ...
            if (FilesystemExtensions.FileExists(symbolsPath)) {
