...
namespace Microsoft.PowerShell.OneGet.CmdLets {
    ...
    public class InstallPackage : FindInstallCmdlet {
        ...
        private Hashtable _installationOptions;
        ...
        public Hashtable InstallationOptions {
            get {
                if (_installationOptions == null) {
                    _installationOptions = new Hashtable();
                }

                return _installationOptions;
            }
            set {
                ...
                if (value != null) {
                    foreach (var k in value.Keys) {
                        _installationOptions[k] = value[k];
                    }
                }
            }
        }
        ...
        public override bool EndProcessingAsync() {
            ...
            if (noMatchNames.Any()) {
                ...
                foreach (var name in noMatchNames) {
                    Event<Error>.Raise("No Package Found", name);
                }
            ...
            foreach (var set in resultsPerName.Values.Where(set => set.Count > 1)) {
                ...
                foreach (var name in noMatchNames) {
                    Event<Error>.Raise("Ambiguity", " '{0}' matches multiple packages '{1}'", new[] {
                        name
                    });
                }
            }
        ...
        private bool InstallPackages(SoftwareIdentity[] packagesToInstall) {
            ...
            foreach (var pkg in packagesToInstall) {
                ...
                var provider = HostApi.SelectProviders(pkg.ProviderName).FirstOrDefault();
                ...
                try {
                    ...
                    e.Dump();
                    Event<Error>.Raise("Installation Failure", pkg.Name);
                    return false;
                }
            }
        ...
        public override bool ConsumeDynamicParameters() {
            ...
            foreach (
                var rdp in DynamicParameters.Keys.Select(d => DynamicParameters[d]).Where(rdp => rdp.IsSet)) {
                if (
                    rdp.ParameterType == typeof (SwitchParameter)) {
                    InstallationOptions[rdp.Name] = true;
                } else {
                    InstallationOptions[rdp.Name] = rdp.Value;
                }
            }
            // then let the base do the rest
            return base.ConsumeDynamicParameters();
        }

        public override bool GenerateDynamicParameters() {
            // do install options first
            // if (Provider.Is() || !Source.IsNullOrEmpty()) {
            foreach (var iod in _providers.Value.SelectMany(provider => provider.GetInstallationOptionDefinitions(Invoke))) {
                DynamicParameters.Add(iod.Name, iod.CreateRuntimeDynamicParameter());
            }

            return base.GenerateDynamicParameters();
        }
