...
namespace Microsoft.PowerShell.OneGet.CmdLets {
    ...
    public class FindInstallCmdlet : PackagingCmdlet {
        ...
        internal FindInstallCmdlet() {
            ...
            _providers = new Lazy<IEnumerable<Provider>>(() => HostApi.SelectProviders(Provider, Source));
        }
        internal bool FindViaUri(Provider provider, string packageuri, Action<SoftwareIdentity> onPackageFound) {
            var found = false;
            if (Uri.IsWellFormedUriString(packageuri, UriKind.Absolute)) {
                using (var packages = CancelWhenStopped(provider.FindPackageByUri(packageuri, Invoke))) {
        ...
        internal bool FindViaFile(Provider provider, string filePath, Action<SoftwareIdentity> onPackageFound) {
            var found = false;
                ...
                try {
                    ...
                    if (files.Any()) {
                        ...
                        foreach (var file in files) {
                            ...
                            using (var packages = CancelWhenStopped(provider.FindPackageByFile(file, Invoke))) {
                                foreach (var p in packages) {
                                    foundThisFile = true;
                                    found = true;
                                    onPackageFound(p);
                            ...
                            if (foundThisFile == false) {
                                ...
                                Event<Warning>.Raise("Package File Not Recognized", file);
                            }
                        }
                    }
                } catch {
        ...
        public override bool GenerateDynamicParameters() {
            ...
            foreach (var md in _providers.Value.SelectMany(provider => provider.GetMetadataDefinitions(Invoke))) {
                DynamicParameters.Add(md.Name, md.CreateRuntimeDynamicParameter());
            }
            return true;
        }

        internal bool FindViaName(Provider provider, string name, Action<SoftwareIdentity> onPackageFound) {
            var found = false;

            using (var packages = CancelWhenStopped(provider.FindPackage(name, RequiredVersion, MinimumVersion, MaximumVersion, Invoke))) {
                foreach (var p in packages) {
                    found = true;
                    onPackageFound(p);
                }
